...
public interface TradeInviteRepository extends JpaRepository<TradeInvite, Long> {
    ...
    @Query("SELECT ti FROM TradeInvite ti WHERE (ti.requester.id = :userId AND ti.accepted = true AND ti.post.active = false) OR ti.post.id IN (SELECT p.id FROM Post p WHERE p.user.id = :userId AND p.active = true)")
